        ...
        move(ev) {
            ...
            this.requestFram = requestAnimationFrame(() => {
                if (that.$refs.content.scrollTop != that.move_last ) {
                    ...
                    if (that.direction) {
                        this.move_last && that.$refs.content.scrollTop == 0 ? window.scrollTo(0, 0) : ''
                    } else {
                        this.move_last && that.$refs.content.scrollTop == 0 ? window.scrollTo(0, getOffsetTop(that.$refs.content)) : ''
                    }
                    that.get_curr_cate()
                    that.move_start = 0;
                    that.move_last = 0;
